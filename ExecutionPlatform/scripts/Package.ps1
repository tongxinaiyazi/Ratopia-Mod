...
$solutionPath = Join-Path $projectRoot 'ExecutionPlatform.sln'
$projectPath = Join-Path $projectRoot 'src\ExecutionPlatform\ExecutionPlatform.csproj'
$buildOutput = Join-Path $projectRoot 'src\ExecutionPlatform\bin\Release\net472\ExecutionPlatform.dll'
$stageDir = Join-Path $projectRoot 'artifacts\package'
$pluginStageDir = Join-Path $stageDir 'BepInEx\plugins\ExecutionPlatform'
$distDir = Join-Path $projectRoot 'dist'
$zipPath = Join-Path $distDir 'ExecutionPlatform-v0.1.1-BepInEx5.zip'
...
Copy-Item -LiteralPath $buildOutput -Destination (Join-Path $pluginStageDir 'ExecutionPlatform.dll')
...
& $packageValidator -Path $zipPath -ExpectedPluginName 'ExecutionPlatform'
if ($LASTEXITCODE -ne 0) {
