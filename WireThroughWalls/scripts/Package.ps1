...
$solutionPath = Join-Path $projectRoot 'WireThroughWalls.sln'
$projectPath = Join-Path $projectRoot 'src\WireThroughWalls\WireThroughWalls.csproj'
$builtDll = Join-Path $projectRoot 'src\WireThroughWalls\bin\Release\net472\WireThroughWalls.dll'
$readmePath = Join-Path $projectRoot 'README.md'
$distRoot = Join-Path $projectRoot 'dist'
$packageRoot = Join-Path $distRoot 'package'
$pluginRoot = Join-Path $packageRoot 'BepInEx\plugins\WireThroughWalls'
$archivePath = Join-Path $distRoot '电线可穿墙-v0.1.3-BepInEx5.zip'
...
Copy-Item -LiteralPath $builtDll -Destination (Join-Path $pluginRoot 'WireThroughWalls.dll')
