...
$solution = Join-Path $projectRoot 'EquipmentReforgeDodge.sln'
$pluginProject = Join-Path $projectRoot 'src\EquipmentReforgeDodge\EquipmentReforgeDodge.csproj'
...
$pluginStageDir = Join-Path $stageDir 'BepInEx\plugins\EquipmentReforgeDodge'
$zipPath = Join-Path $distDir '装备重铸闪避属性-v0.1.0-BepInEx5.zip'
$pluginDll = Join-Path $projectRoot 'src\EquipmentReforgeDodge\bin\Release\net472\EquipmentReforgeDodge.dll'
...
Copy-Item -LiteralPath $pluginDll -Destination (Join-Path $pluginStageDir 'EquipmentReforgeDodge.dll')
...
try {
    ...
    $expectedEntries = @(
        'BepInEx/plugins/EquipmentReforgeDodge/EquipmentReforgeDodge.dll',
        'README.md'
    )
